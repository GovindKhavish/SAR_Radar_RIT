...
chirp_count = {i: 0 for i in range(4)} 

for row_idx in range(0, num_rows, 3):  # Every third row
    chirp_idx = np.random.randint(0, 4)  
    chirp_signal = chirp_signals[chirp_idx]
    num_samples = num_samples_list[chirp_idx]
    
    start_idx = np.random.randint(0, num_cols - num_samples)  
    end_idx = start_idx + num_samples
    
    radar_data[row_idx, start_idx:end_idx] += chirp_signal 
    chirp_count[chirp_idx] += 1 
...
for idx_n in range(start_idx, end_idx + 1):
    ...
    slow_time_offset = (idx_n / fs)*1e6 #microseconds
...
with conn:
    ...
    CREATE TABLE IF NOT EXISTS pulse_data (
        ...
        pulse_duration REAL,
        start_time_iq INTEGER,
        end_time_iq INTEGER
    )
    """)

    # Insert the extracted pulse data
        ...
        for params in params_list:
            cursor.execute(
                """INSERT INTO pulse_data (
                    ...
                    start_time_iq, end_time_iq
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    int(params["pulse_number"]),
                    params["bandwidth"],
                    params["center_frequency"],
                    params["chirp_rate"],
                    int(params["start_time_index"]),
                    int(params["end_time_index"]),
                    params["adjusted_start_time"],
                    params["adjusted_end_time"],
                    params["pulse_duration"],
                    int(params["start_time_iq"]),
                    int(params["end_time_iq"])
                )
            )
...
conn.close()
print(f"Pulse characteristics stored in SQLite3 database at {db_path}.")
