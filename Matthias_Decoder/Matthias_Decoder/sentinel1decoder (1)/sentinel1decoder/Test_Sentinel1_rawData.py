...
### Decode the IQ data
#radar_data = l0file.get_burst_data(selected_burst, try_load_from_file=False, save=False);
...
ax  = fig.add_subplot(111);
ax.plot(np.abs(radar_data_raw[831,:]), label='831');
ax.plot(np.abs(radar_data_raw[835,:]), label='835');
ax.plot(np.abs(radar_data_raw[839,:]), label='839');
ax.legend();
ax.set_title(f'{headline} Raw I/Q Sensor Output', fontweight='bold');
...
ax  = fig.add_subplot(111);
ax.plot(np.real(radar_data_raw[831,:]), 'b', label='831 - R');
ax.plot(np.imag(radar_data_raw[831,:]), 'b:', label='831 - I');
#ax.plot(np.real(radar_data_raw[835,:]), label='835');
#ax.plot(np.real(radar_data_raw[839,:]), label='839');
ax.legend();
ax.set_title(f'{headline} Raw I/Q Sensor Output', fontweight='bold');
ax.set_xlabel('Fast Time (down range) [samples]', fontweight='bold');
ax.set_ylabel('Re(Amplitude)', fontweight='bold');
plt.tight_layout(); plt.pause(0.1); plt.show();
...
### Schätzen der Pulsparameter
# chirprate, pulse_duration, fstart, fstop
idx_slowtime = 831; idx_pstart = 3783; idx_pstop = 4609;
sigest  = radar_data_raw[idx_slowtime, np.arange(idx_pstart, idx_pstop+1)];
